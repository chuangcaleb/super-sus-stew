...
# Azure Bluet				Blindness 15 + Night Vision 16
...
# Azure Bluet -> Blindness + Night Vision 30s
execute if entity @e[tag=cc.ss.active_cow,sort=nearest,limit=1,scores={cc.ss.EffectId=15}] run item modify entity @s weapon.mainhand cchesed:susstw/overworld/blindness

# Blue Orchid & Dandelion -> Haste 30s
execute if entity @e[tag=cc.ss.active_cow,sort=nearest,limit=1,scores={cc.ss.EffectId=23}] run item modify entity @s weapon.mainhand cchesed:susstw/overworld/absorption
